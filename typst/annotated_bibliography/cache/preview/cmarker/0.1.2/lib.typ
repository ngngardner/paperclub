...
#let render(
  markdown, smart-punctuation: true, blockquote: none, math: none, h1-level: 1, raw-typst: true, scope: (:), show-source: false,
) = {
  ...
  if math != none {
    options += 0b00001000
    scope += (
      inlinemath: math.with(block: false), displaymath: math.with(block: true),
    )
  }
